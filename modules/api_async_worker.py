...
class AsyncTask:
    def __init__(self, request: GenerateRequest):
        ...
        self.style_selections = request.style_selections
        self.performance_selection = Performance(request.performance_selection)
        self.steps = self.performance_selection.steps()
        self.original_steps = self.steps
        self.aspect_ratios_selection = request.aspect_ratios_selection 
        self.image_number = request.image_number
        self.seed = request.seed
        self.base_model_name = request.base_model_name
        self.refiner_model_name = request.refiner_model_name if hasattr(request, "refiner_model_name") else "None"
        
        # 设置默认值
        self.generate_image_grid = False
        self.output_format = "png"
        self.read_wildcards_in_order = True
        self.sharpness = request.sharpness if hasattr(request, 'sharpness') else 2.0
        self.cfg_scale = request.guidance_scale if hasattr(request, 'guidance_scale') else 7.0
        self.refiner_switch = request.refiner_switch if hasattr(request, 'refiner_switch') else 0.5

        # LoRA 相关
        if hasattr(request, 'loras') and request.loras:
            self.loras = get_enabled_loras([
                (True, str(lora.name), float(lora.weight)) 
        ...
        self.disable_intermediate_results = False
        ...
        self.clip_skip = request.clip_skip if hasattr(request, 'clip_skip') else 1
        self.sampler_name = request.sampler if hasattr(request, 'sampler') else "dpmpp_2m"
        self.scheduler_name = request.scheduler if hasattr(request, 'scheduler') else "karras"
        self.vae_name = 'Default (model)'  # Default VAE
        
        # Overwrite 参数
        self.overwrite_step = 0
        self.overwrite_switch = 0
        self.overwrite_width = 0
        self.overwrite_height = 0
        self.overwrite_vary_strength = 0
        self.overwrite_upscale_strength = 0
        
        # Advanced 参数
        self.debugging_cn_preprocessor = False
        self.skipping_cn_preprocessor = False
        self.controlnet_softness = 0.25
        self.canny_low_threshold = 0.3  
        self.canny_high_threshold = 0.7
        ...
        self.inpaint_advanced_masking_checkbox = False
        self.invert_mask_checkbox = False
        self.inpaint_erode_or_dilate = 0

        # Enhancement 参数
        self.enhance_input_image = None
        self.enhance_checkbox = False
        self.enhance_uov_method = disabled.casefold()
        self.enhance_uov_processing_order = 'before'
        self.enhance_uov_prompt_type = 'last'
        self.enhance_ctrls = []
        self.debugging_dino = False
        self.dino_erode_or_dilate = 0
        self.debugging_enhance_masks_checkbox = False
        ...
        self.save_metadata_to_images = True
